    ...
    <div className="mt-10 max-w-2xl">
      ...
      {off > on && (
        <p className="text-xs mt-3" style={{ color: '#d2a24c' }}>
          band inverted — the bid side is pricing this parcel above the ask side. Since model 2.0.0
          that should be unreachable: one fit, one is-ask coefficient, ask = bid x that. Seeing this
          means the coefficients were regenerated by something other than the current fit_hedonic.py.
          Worth checking before reading anything into the number.
        </p>
      )}
